...
def Process_data(df,CEGH_Filter,gene,Variant_ID):
...
def Process_variant_ID_data(df,CEGH_Filter):
...
def Searches(lista:list):
     ...
     if 'Number of Hemizygous' in final_data.columns:
         final_data['Number of Hemizygous'] = final_data['Number of Hemizygous'].fillna(0)
...
def Search_gene(version:str,gene:str,CEGH_Filter= False,GATK_PASS= False,Variant_ID=False, Process= True):
        ...
        if Process==True:
            result = Process_data(response,CEGH_Filter,gene,Variant_ID,)
...
def Variant_ID(version:str,variant:str,CEGH_Filter= False,GATK_PASS= False,Process= True):
    ...
    if Process==True:
       result = Process_variant_ID_data(response,CEGH_Filter)
       data= Dataframe_adjust(result)
...
def Variant_ID_biovar(dataframe,version):
    ...
    for i in range(0,len(dataframe)):
      if dataframe['Alternative'][i]=='-': 
        next_nt=indel_info_genome(int(dataframe['Chromosome'][i]),version, int(dataframe['Position'][i])+1)
        variant_ID_biovar = dataframe['Chromosome'][i]+'-'+dataframe['Position'][i]+'-'+dataframe['Reference'][i]+'-'+dataframe['Reference'][i]+next_nt
        frequency = dataframe['Allele Frequency'][i]
        list1.append([variant_ID_biovar,frequency])

      elif dataframe['Reference'][i]=='-': 
        prev_nt=indel_info_genome(int(dataframe['Chromosome'][i]),version, int(dataframe['Position'][i]))
